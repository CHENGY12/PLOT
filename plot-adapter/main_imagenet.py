...
def run_tip_adapter_F(cfg, clip_model, train_loader_F,test_loader):

    adapter = nn.Linear(cfg['cache_keys'].shape[0], cfg['cache_keys'].shape[1]*cfg['N'], bias=False).to(clip_model.dtype).cuda() 
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        if acc > best_acc:
            ...
            cfg['logits_save']= torch.cat(tip_logits_test_list_fuse)
    ...
    cfg['val_loader'] = test_loader
    best_acc1, best_beta, best_alpha = search_hp_imagenet(cfg,clip_model, adapter=adapter) 
    best_acc = max (best_acc1, best_acc)
    print(f"**** Finally, Tip-Adapter-F's best test accuracy: {best_acc:.2f}, with beta: {best_beta:.2f} and alpha: {best_alpha:.2f} ****\n")
...
def main():
    ...
    plot_model= clip.load_clip_to_cpu(cfg['backbone'])
    model_dir = os.path.join(cache_dir, 'shot'+str(cfg['shots'])) 
    model = CustomCLIP(imagenet.classnames, plot_model)
    ...
    cfg['val_features'] = test_features
    cfg['val_labels'] = test_labels
    cfg['val_featuremaps'] = test_featuremaps
    cfg['val_logits'] = test_logits

    # pdb.set_trace()

    cfg['M'] = 49
    cfg['N'] = 2
    cfg['classnames'] = imagenet.classnames
    # ------------------------------------------ Tip-Adapter-F ------------------------------------------
    
    run_tip_adapter_F(cfg, model, train_loader_F,test_loader)
